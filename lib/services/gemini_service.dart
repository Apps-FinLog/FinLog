...
class GeminiService {
  ...
  Future<Map<String, dynamic>> parseExpense(String userInput) async {
    // Check if dotenv is loaded and API key exists
    if (!dotenv.isInitialized) {
      throw Exception(
        'Environment variables not initialized. Please ensure dotenv.load() is called in main().',
      );
    }
    ...
    final String prompt = """
You are an expense journal assistant. Your task is to parse the following user input and extract the expense details into a JSON object. If a field is not explicitly mentioned, infer it if possible (e.g., today's date for 'date', 'cash' for 'paymentMethod' if not specified), otherwise omit it or use a reasonable default. make the category tailored to indonesia users, usually involved in food, transportation, some of the foods are 'makan', 'minum', 'snack', 'kopi', 'makanan berat', 'makanan ringan', 'transportasi', 'belanja', 'hiburan', 'tagihan', 'lainnya'. If the user input is not clear or does not contain enough information, return an empty JSON object. act as a helpful and also friendly neighbourhood financial assistant that makes the user feel comfortable to input their expense
...
also add some easter egg here, like a joke if the user prompt something silly reply with response for example like this:
JSON structure:
{"description": <string>
"amount": <number>,
  "currency": <string>,
  ...
  "date": <string, YYYY-MM-DD>,
  "paymentMethod": <string>} 
make the other field be as creative as possible, but still follow the JSON structure. buat amoutnnya bombastis 
